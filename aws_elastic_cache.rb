...
opts = Trollop::options do
  opt :cache_cluster_id,"name of the cluster", :type => :string                       #default nil, required true "Set the name of the cache cluster"                     opt :num_cache_nodes, "number of nodes", :default => 1                              #set the number of cache nodes, default is 1
...
aws.create_cache_cluster({:cache_cluster_id => opts[:cache_cluster_id],:num_cache_nodes => opts[:num_cache_nodes],:cache_node_type => opts[:cache_node_type],
:engine => opts[:engine],:port => opts[:port]})
